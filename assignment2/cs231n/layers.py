...
def affine_forward(x, w, b):
    ...
    out = np.dot(x_reshaped, w) + b             # [NxD]*[DxM] + [M,1]
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_n = (x - running_mean)/np.sqrt(running_var)
        out = gamma * x_n + beta
...
def batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * cache['x_n'], axis=1)
    ...
    dvar = -0.5 * np.sum( dxn*(x-mu), axis=0 ) / (std**3)

    dmu = -np.sum(dxn/std, axis=0) + (-2/N)*dvar*np.sum(x-mu, axis=0)

    dx = dxn/std + (2/N)*dvar*(x-mu) + dmu/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxn = dout * cache['gamma']
    dxn_sum = np.sum(dxn, axis=0)
    dx = dxn - dxn_sum/N - np.sum(dxn*x_n, axis=0) * x_n/N
    dx /= cache['std']
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    outH = 1 + (H + 2*pad - FH)//stride
    outW = 1 + (W + 2*pad - FW)//stride
    ...
    for n in range(N):
        for f in range(F):
            for j in range(0, outH):
                for i in range(0, outW):
                    out[n, f, j, i] = np.sum( x_pad[n,:, j*stride:(j*stride+FH), i*stride:(i*stride+FW)]*w[f,:,:,:] ) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            ...
            for j in range(outH):
                for i in range(outW):
                    dw[f] += x_pad[n, :, j*stride:(j*stride+FH), i*stride:(i*stride+FW) ] * dout[n, f, j, i]
                    dx_pad[n, :, j*stride:(j*stride+FH), i*stride:(i*stride+FW) ] += w[f] * dout[n, f, j ,i]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x = np.transpose(x, (0,2,3,1)).reshape(N*W*H, C)
